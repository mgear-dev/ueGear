...
def sequence_to_dict(sequence):
	...
	for master_trak in sequence.find_master_tracks_by_type(unreal.MovieSceneTrack):
...
def get_bound_objects(sequence):
	...
	bound_objects = unreal.SequencerTools.get_bound_objects(world, sequence, sequence.get_bindings(), sequence_range)
...
def export_fbx_sequence(sequence, directory, root_sequence=None, fbx_filename='', export_options=None):
	"""
	...
	fbx_path = helpers.clean_path(os.path.join(directory, '{}.fbx'.format(fbx_filename or sequence.get_name())))
	world = unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world()
	...
	export_fbx_params = unreal.SequencerExportFBXParams(
		world, sequence, root_sequence, bindings, master_tracks, fbx_export_options, fbx_path)
