...
def auto_tag(asset=None, remove=False, save_assets=False):
	...
	for asset in assets:
		...
		if asset_class == unreal.SkeletalMesh.static_class():
			remove_tag(asset) if remove else apply_tag(asset, attribute_value=TagTypes.SkeletalMesh)
		elif asset_class == unreal.StaticMesh.static_class():
			remove_tag(asset) if remove else apply_tag(asset, attribute_value=TagTypes.StaticMesh)
		elif asset_class == unreal.Skeleton.static_class():
			remove_tag(asset) if remove else apply_tag(asset, attribute_value=TagTypes.Skeleton)

		remove_tag(asset, TAG_ASSET_ID_ATTR_NAME) if remove else apply_tag(asset, TAG_ASSET_ID_ATTR_NAME, asset_name)
...
def apply_tag(asset=None, attribute_name=TAG_ASSET_TYPE_ATTR_NAME, attribute_value=''):
	...
	for asset in assets:
		...
		if attribute_value:
			...
			unreal.log('Tagged "{}.{}" as empty.'.format(asset, attribute_name))
...
def remove_tag(asset=None, attribute_name=TAG_ASSET_TYPE_ATTR_NAME):
	...
	for asset in assets:
		...
		unreal.log('Removed attribute {} from "{}"'.format(attribute_name, asset))
